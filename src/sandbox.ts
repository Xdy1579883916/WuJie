...
import type { loadErrorHandler, plugin, ScriptObjectLoader } from './index'
import {
...
import { WUJIE_CONFIG } from './constant'
import { removeEventListener } from './effect'
import { appEventObjMap, EventBus } from './event'
import {
  iframeGenerator,
...
import { localGenerator, proxyGenerator } from './proxy'
import {
  clearChild,
...
import { clearInactiveAppUrl, syncUrlToIframe, syncUrlToWindow } from './sync'
import { appRouteParse, eventTrigger, getAbsolutePath, isFunction, requestIdleCallback, wujieSupport } from './utils'
...
export default class Wujie {
  ...
  public patchCssRules(): void {
    if (this.degrade)
      return
    if (this.shadowRoot.host.hasAttribute(WUJIE_CONFIG.DATA_ATTACH_CSS_FLAG))
      return
    ...
    && this.shadowRoot.host.setAttribute(WUJIE_CONFIG.DATA_ATTACH_CSS_FLAG, '')
  }
