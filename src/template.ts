...
function isValidJavaScriptType(type) {
  ...
  return !type || handleTypes.includes(type)
}
...
export function genLinkReplaceSymbol(linkHref, preloadOrPrefetch = false) {
  return `<!-- ${preloadOrPrefetch ? 'prefetch/preload/modulepreload' : ''} link ${linkHref} replaced by wujie -->`
}
export const getInlineStyleReplaceSymbol = index => `<!-- inline-style-${index} replaced by wujie -->`
export function genScriptReplaceSymbol(scriptSrc, type = '') {
  return `<!-- ${type} script ${scriptSrc} replaced by wujie -->`
}
...
export function genModuleScriptReplaceSymbol(scriptSrc, moduleSupport) {
  return `<!-- ${moduleSupport ? 'nomodule' : 'module'} script ${scriptSrc} ignored by wujie -->`
}
