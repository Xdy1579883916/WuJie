...
import type { ScriptObjectLoader } from './index'
...
import { WUJIE_CONFIG, WUJIE_TIPS_SCRIPT_ERROR_REQUESTED } from './constant'
...
function patchDocumentEffect(iframeWindow: Window): void {
  ...
  // 处理onEvent
  const elementOnEvents = Object.keys(iframeWindow.HTMLElement.prototype).filter(ele => ele.startsWith('on'))
  const documentOnEvent = Object.keys(iframeWindow.Document.prototype)
    .filter(ele => ele.startsWith('on'))
...
export function iframeGenerator(
  ...
  const iframe = window.document.createElement('iframe')
  const attrsMerge = { src: mainHostPath, style: 'display: none', ...attrs, name: sandbox.id, [WUJIE_CONFIG.DATA_FLAG]: '' }
