...
export function defineWujieWebComponent() {
  ...
  if (customElements && !customElements?.get(WUJIE_CONFIG.APP)) {
    class WujieApp extends HTMLElement {
      connectedCallback(): void {
        if (this.shadowRoot)
          return
        const shadowRoot = this.attachShadow({ mode: 'open' })
        const sandbox = getWujieById(this.getAttribute(WUJIE_CONFIG.APP_ID))
    ...
    customElements?.define(WUJIE_CONFIG.APP, WujieApp)
  }
}

export function createWujieWebComponent(id: string): HTMLElement {
  const contentElement = window.document.createElement(WUJIE_CONFIG.APP)
  contentElement.setAttribute(WUJIE_CONFIG.APP_ID, id)
...
export function renderElementToContainer(
  ...
  if (container && !container.contains(element)) {
    ...
    if (!container.querySelector(`div[${WUJIE_CONFIG.LOADING_DATA_FLAG}]`)) {
      // 清除内容
...
export function addLoading(el: string | HTMLElement, loading: HTMLElement): void {
  ...
  if (containerStyles.position === 'static') {
    container.setAttribute(WUJIE_CONFIG.CONTAINER_POSITION_DATA_FLAG, containerStyles.position)
    container.setAttribute(
      WUJIE_CONFIG.CONTAINER_OVERFLOW_DATA_FLAG,
  ...
  loadingContainer.setAttribute(WUJIE_CONFIG.LOADING_DATA_FLAG, '')
...
export function removeLoading(el: HTMLElement): void {
  // 去除容器设置的样式
  const positionFlag = el.getAttribute(WUJIE_CONFIG.CONTAINER_POSITION_DATA_FLAG)
  const overflowFlag = el.getAttribute(WUJIE_CONFIG.CONTAINER_OVERFLOW_DATA_FLAG)
  if (positionFlag)
    el.style.removeProperty('position')
  if (overflowFlag !== null) {
    overflowFlag ? el.style.setProperty('overflow', overflowFlag) : el.style.removeProperty('overflow')
  }
  el.removeAttribute(WUJIE_CONFIG.CONTAINER_POSITION_DATA_FLAG)
  el.removeAttribute(WUJIE_CONFIG.CONTAINER_OVERFLOW_DATA_FLAG)
  const loadingContainer = el.querySelector(`div[${WUJIE_CONFIG.LOADING_DATA_FLAG}]`)
