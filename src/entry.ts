import type { loadErrorHandler, plugin } from './index'
...
import { compose, defaultGetPublicPath, error, getCurUrl, getInlineCode, requestIdleCallback } from './utils'
...
function fetchAssets(src: string, cache: object, fetch: (input: RequestInfo, init?: RequestInit) => Promise<Response>, cssFlag?: boolean, loadError?: loadErrorHandler) {
  return cache[src]
    || (cache[src] = fetch(src)
      .then((response) => {
