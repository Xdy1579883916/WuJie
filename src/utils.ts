import type { cacheOptions, plugin } from './index'
import {
  WUJIE_CONFIG,
  WUJIE_TIPS_NO_URL,
  WUJIE_TIPS_STOP_APP,
...
export function getDegradeIframe(id: string): HTMLIFrameElement {
  return window.document.querySelector(`iframe[${WUJIE_CONFIG.APP_ID}="${id}"]`)
}
...
export function appRouteParse(url: string): {
  ...
  if (!appRoutePath.startsWith('/'))
    appRoutePath = `/${appRoutePath}` // hack ie
...
export function setTagToScript(element: HTMLScriptElement, tag?: string): void {
  if (isScriptElement(element)) {
    ...
    element.setAttribute(WUJIE_CONFIG.SCRIPT_ID, scriptTag)
  }
}

export function getTagFromScript(element: HTMLScriptElement): string | null {
  if (isScriptElement(element)) {
    return element.getAttribute(WUJIE_CONFIG.SCRIPT_ID)
  }
  return null
}
